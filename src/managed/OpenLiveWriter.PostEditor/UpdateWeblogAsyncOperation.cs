            ...
            public void ThreadStart()
            {
                foreach (string url in _pingUrls)
                {
                    try
                    {
                        ...
                        {
                            // Send a simple XML-RPC ping using raw HTTP POST
                            string xmlBody = string.Format(CultureInfo.InvariantCulture,
                                "<?xml version=\"1.0\"?><methodCall><methodName>weblogUpdates.ping</methodName><params><param><value><string>{0}</string></value></param><param><value><string>{1}</string></value></param></params></methodCall>",
                                System.Security.SecurityElement.Escape(_blogName),
                                System.Security.SecurityElement.Escape(_blogUrl));

                            System.Net.HttpWebRequest request = HttpRequestHelper.CreateHttpWebRequest(url, true);
                            request.Method = "POST";
                            request.ContentType = "text/xml";
                            request.UserAgent = ApplicationEnvironment.UserAgent;
                            byte[] data = System.Text.Encoding.UTF8.GetBytes(xmlBody);
